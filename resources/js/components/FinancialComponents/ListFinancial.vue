...
export default {
    ...
    methods: {
        async carregarLancamentos() {
            this.loading = true;
            try {
                const response = await axios.get('/financial/list');
...
<template>
    <div>
        ...
        <template v-else>
            <div v-if="lancamentos.length === 0" class="p-4 text-center text-yellow-500">
            ...
            <table v-else class="min-w-full divide-y border text-sm">
                ...
                <tbody>
                    <tr v-for="item in lancamentos" :key="item.id" class="border-t hover:bg-gray-800 transition-colors">
                        <td class="px-4 py-2">
                        ...
                        <td class="px-4 py-2 flex gap-2">
                            <button @click="editarLancamento(item)"
                                class="p-1 text-blue-400 hover:text-blue-300 transition-colors" title="Editar">
                            ...
                            <button @click="excluirLancamento(item)"
                                class="p-1 text-red-400 hover:text-red-300 transition-colors" title="Excluir">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
